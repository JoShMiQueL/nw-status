...
import type { ServerStatus, ServerStatistics, StatusHistoryEntry } from '../../domain/types';
...
export class JsonHistoryRepository implements IHistoryRepository {
  ...
  async saveStatus(status: ServerStatus): Promise<void> {
    try {
      ...
      history[status.name]!.push(entry);

      // Keep only last N entries
      if (history[status.name]!.length > this.maxHistoryEntries) {
        history[status.name] = history[status.name]!.slice(-this.maxHistoryEntries);
      }
  ...
  async getStatistics(serverName: string): Promise<ServerStatistics | null> {
    try {
      ...
      const onlineEntries = entries.filter((e: StatusHistoryEntry) => e.status === ServerState.ONLINE);
      const transferAvailable = entries.filter((e: StatusHistoryEntry) => e.canTransferTo);

      const lastOnlineEntry = [...entries].reverse().find((e: StatusHistoryEntry) => e.status === ServerState.ONLINE);
      const lastOfflineEntry = entries.find((e: StatusHistoryEntry) => e.status === ServerState.OFFLINE);
      ...
      console.error('❌ Error calculating statistics:', error instanceof Error ? error.message : error);
      return null;
    }
  }
  ...
  private async loadHistory(): Promise<HistoryData> {
    try {
      ...
      console.warn('⚠️ Could not load history file:', error instanceof Error ? error.message : error);
    }
  ...
  private async saveHistory(history: HistoryData): Promise<void> {
    try {
      ...
      console.error('❌ Could not save history file:', error instanceof Error ? error.message : error);
    }
  }
